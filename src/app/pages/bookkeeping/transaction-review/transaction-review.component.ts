...
@Component({
  ...
  standalone: true,
  imports: [CurrencyPipe, JsonPipe, RouterModule, MatButtonModule, MatCardModule],
...
export class TransactionReviewComponent {
  ...
  private readonly analytics$ = toObservable(this.refreshTick).pipe(
    switchMap(() => this.intelligenceService.getAnalytics('收入'))
  );
  private readonly recurring$ = toObservable(this.refreshTick).pipe(
    switchMap(() => this.intelligenceService.getRecurringCandidates())
  );
  readonly analytics = toSignal(this.analytics$);
  readonly recurring = toSignal(this.recurring$, { initialValue: [] });
